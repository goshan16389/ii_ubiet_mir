...
Разработка модели для многоклассовой классификации состояний растений (38 классов болезней и здоровых состояний) на основе изображений из датасета PlantVillage.

## Цель и мотивация
Цель — создать точную модель для выявления болезней растений, что поможет фермерам реагировать на проблемы, снижая потери урожая. Мотивация — автоматизация диагностики в сельском хозяйстве для повышения эффективности и устойчивости производства.

## Использованные данные
Данные взяты из датасета PlantVillage, загруженного через TensorFlow Datasets (tfds.load('plant_village')). Датасет содержит изображения листьев растений с различными болезнями и здоровыми образцами (38 классов). Источник: [TensorFlow Datasets - Plant Village](https://www.tensorflow.org/datasets/catalog/plant_village).
<img width="900" height="900" alt="image" src="https://github.com/user-attachments/assets/6ff9979c-f0ac-4b58-88eb-777f5f66ea36" />


## Архитектура модели и обоснование выбора
Модель основана на EfficientNetB0 (предобученная на ImageNet) с добавлением GlobalAveragePooling2D, Dense слоя (256 нейронов с ReLU) и финального Dense слоя для 38 классов (softmax).
EfficientNet обеспечивает высокую точность при низких вычислительных затратах, подходя для задач компьютерного зрения в агрономии.
